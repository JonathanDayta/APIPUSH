...
select [ID] ,[SMSId] ,[Type] , [ValidTime] ,[BeginTime] ,[UserID] ,[Content]
 ...
 from SMS {0};", sqlWhere);
        ...
        {
            string sql = string.Format(@"insert into SMS(SMSId,[Type],ValidTime,BeginTime,UserID,Content
    ) values (@SMSId,@Type,@ValidTime,@BeginTime,@UserID,@Content);");
        ...
        {
            string sql = string.Format(@"delete from SMS where smsID='{0}';", smsID);
        ...
        {
            string sql = string.Format(@"update SMS set 
    SMSId = @SMSId , 
    [Type] = @Type ,            
    ValidTime = @ValidTime,  
    BeginTime = @BeginTime,  
