...
namespace Dal
{
    ...
    public class TmpFaceDal
    {
        ...
        {
            string sql = string.Format(@"select * from TmpFace where pin ='{0}';", pin);
        ...
        {
            string sql = string.Format(@"select count(*) from TmpFace where pin='{0}' and ver='{1}'", pin.ToLower(), ver.ToLower());
...
insert into TmpFace(Pin,Fid,Size,Valid,Tmp,Ver,DischargeDate)
 values(@Pin,@Fid,@Size,@Valid,@Tmp,@Ver,@DischargeDate);
");
            ...
            SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@Ver", model.Ver != null ? model.Ver : ""),
        ...
        {
            string sql = string.Format(@"delete from TmpFace  where Pin in ({0});", Tools.UnionString(pins));
