...
namespace Dal
{
    ...
    public class DeviceCmdDal
    {
        ...
        {
            string sql = string.Format(@"insert into DeviceCmds(DevSN,Content,CommitTime,ReturnValue) values (@DevSN,@Content,@CommitTime,@ReturnValue);");

            SqlParameter[] parameters = {
                        new SqlParameter("@DevSN", dCmd.DevSN) ,
                        new SqlParameter("@Content",  dCmd.Content) ,
                        new SqlParameter("@CommitTime", dCmd.CommitTime) ,
                        new SqlParameter("@ReturnValue", dCmd.ReturnValue)
        ...
        {
            string sql = string.Format(@"select TOP 200 ID, DevSN, Content, CommitTime, TransTime, ResponseTime, ReturnValue from DeviceCmds where DevSN=@DevSN and (ReturnValue is null or len(ReturnValue)=0)");
        ...
        public DataTable GetByTime(DateTime startTime, DateTime endTime, string devSN)
        {
            ...
            string sql = string.Format(@"select * from DeviceCmds where CommitTime>@CommitTime1 and CommitTime<@CommitTime2 {0} order by CommitTime desc", whereDevSN);

            SqlParameter[] parameters = {
                 new SqlParameter("@CommitTime1", startTime),
                 new SqlParameter("@CommitTime2", endTime),
                 new SqlParameter("@DevSN", devSN)
            };
        ...
        {
            string sql = string.Format(@"select * from DeviceCmds order by CommitTime desc");
        ...
        {
            string sql = string.Format(@"update DeviceCmds set ResponseTime = @ResponseTime, ReturnValue = @ReturnValue where ID=@ID");
            ...
            {
                if (validaParametros(contentList[i]))
                {
                    SqlParameter[] parameters = {
                        new SqlParameter("@ResponseTime", Tools.GetDateTimeNowString()),
                        new SqlParameter("@ReturnValue",contentList[i]),
                        new SqlParameter("@ID",contentList[i].Split('&')[0].Split('=')[1])
                    };
        ...
        /**
         * Valida parametros de tipo cadena
         * param name="param" Parametro a validar
         */
        private Boolean validaParametros(String param)
        {
            string[] contentElem = param.Split('=');
            Boolean esValido = false;
            for (int i = 0, j = contentElem.Length; i < j && contentElem[i].Length > 0; i++)
            {
                if (contentElem[i].Length == 2 && contentElem[i].Equals("ID"))
                {
                    esValido = true;
                    break;
                }
        ...
        {
            string sql = string.Format(@"delete from DeviceCmds  where id in ({0})", Tools.UnionString(listId));
