...
namespace Attendance
{
    ...
    public partial class UCUser : UserControl
    {
        ...
        private AssignmentUserBll _auBll = new AssignmentUserBll();
        ...
        private void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow item in _CurSelectRows)
            {
                ...
                sName = uName;
                //se agrega usuario y SN a la tabla relacional
                save_assignmentUsr(cmbDevice.Text, curPin);
        ...
        {
            string searchValue = this.txtBoxSearch.Text;
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in this.dgvUser.Rows)
                {
                    for (int i = 0; i < row.Cells.Count; i++)
                    {
                        if (row.Cells[i].Value != null && row.Cells[i].Value.ToString().Contains(searchValue))
                        {
                            ...
                            this.dgvUser.Rows[rowIndex].Selected = true;
                            valueResult = true;
                            break;
                        }
                    }
                ...
                if (!valueResult)
                {
                    MessageBox.Show("Unable to find " + this.txtBoxSearch.Text, "Not Found");
                    return;
                }
            }
            catch (Exception exc)
