...
namespace Attendance
{
    ...
    public partial class UCCommInfo : UserControl
    {
        ...
        public void AddCommInfo(string info, int Mode)
        {
            if (_IsPause)
                return;
            string onlyStartStop = string.Empty;
            ...
            else if (3 == Mode)
            {
                ...
                onlyStartStop = string.Format("Sever Start:  {0}\r\n{1}\r\n", strNow, onlyStartStop);
            }
            else if (4 == Mode)
            {
                info = string.Format("Sever Stop:  {0}\r\n{1}\r\n", strNow, info);
                onlyStartStop = string.Format("Sever Stop:  {0}\r\n{1}\r\n", strNow, onlyStartStop);
            }
            this.rtxtCommInfo.AppendText(onlyStartStop);
