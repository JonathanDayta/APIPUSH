...
namespace Attendance
{
    ...
    public class ListenClient
    {
        ...
        /// new punch event
        /// </summary>
        public event Action<CheckinoutModel> OnNewCheckLog;
        ...
        public void StartListening()
        {
            try
            {
                ...
                System.Console.WriteLine("Server IP: " + serverIP + ":"+port);
                ...
                while (listening)
                {
                    ...
                    {
                        System.Console.WriteLine("ERROR de conexion: " + ex.StackTrace);
                    }
                }
        ...
        {
            System.Console.WriteLine("Ejecutando Analysis >>> " + DateTime.Now);
            ...
            else
            {
                ...
                Log.WriteLogs("UnKnown message from device: " + strReceive);
            }
        }
        ...
        private string GetDeviceInitInfo(DeviceModel device)
        {
            ...
            RetDeviceInfo.AppendFormat("IP Address={0}\n", device.DevIP);
            RetDeviceInfo.AppendFormat("NAME DEVICE={0}\n", device.DevName);
            RetDeviceInfo.AppendFormat("MAC={0}\n", device.DevMac);
            RetDeviceInfo.AppendFormat("Vendor Name={0}\n", device.VendorName);
        ...
        private CheckinoutBll _checkInOutBll = null;
        private CheckinoutBll CheckinoutBll
        {
            get
            {
                if (_checkInOutBll == null)
                    _checkInOutBll = new CheckinoutBll();
                return _checkInOutBll;
            }
        }
        ...
        private void UpDateDeviceInfo(DeviceModel device, string strDevInfo)
        {
            ...
            try
            {
                ...
                //device.LastRequestTime = Tools.GetDateTimeNow();
                device.AttCount = Convert.ToInt32(splitStr[3]);
                device.DevIP = splitStr[4];
            }
            catch
            {
                Log.WriteLogs("Device Info Error:" + strDevInfo);
            }
        ...
        /**
         * Actualiza la fecha de ultima conexion del dispositivo en cuestion 
         * param name="device"
         **/
        private void UpDateDeviceLastRequestTime(DeviceModel device)
        {
            try
            {
                device.LastRequestTime = Tools.GetDateTimeNow();
            ...
            {
                Log.WriteLogs("Device update date Error:" + device.DevSN);
            }
        ...
        /**
         * Actualiza el conteo de FP del dispositivo en cuestion 
         * param name="device"
         **/
        private void UpDateDeviceFpCount(String SNValue, int count)
        {

            DeviceModel device = DeviceBll.Get(SNValue);
            try
            {
                device.FpCount = count;
            }
            catch
            {
                Log.WriteLogs("Device update date Error:" + device.DevSN);
        ...
        private void GetRequestProcess(byte[] bReceive, Socket remoteSocket)
        {
            ...
            {
                //DeviceBll.SetLastRequestTime(SN);
                ...
                UpDateDeviceLastRequestTime(device);
        ...
        private void DeviceCmdProcess(byte[] bReceive, Socket remoteSocket)
        {
            ...
            //Inicia la actualizacion de FPCount
            string fPCountValue = extractData(strReceive, "FPCount=");
            if (fPCountValue != null)
            {
                System.Console.WriteLine("FPCount: " + fPCountValue);
                string serialNumberValue = extractData(strReceive, "~SerialNumber=");
                if (serialNumberValue != null)
                {
                    System.Console.WriteLine("SerialNumber: " + serialNumberValue);
                    UpDateDeviceFpCount(serialNumberValue, Int32.Parse(fPCountValue));
                }
            }
			//Finaliza la actualizacion de FPCount
        ...
        /// <summary>
        /// Extrae los datos solicitados del response al solicitar el INFO del dispositivo 
        /// </summary>
        /// <param name="dataComplete"></param>
        /// <param name="param"></param>
        private string extractData(String dataComplete, String param)
        {
            int index = dataComplete.IndexOfEx(param);
            System.Console.WriteLine("index: " + index);
            if (index != -1) {
                string arrContent = dataComplete.Substring(index);

                string[] contentList = arrContent.Split('\n');
                string data = contentList[0];

                int indexEquals = data.IndexOfEx("=");
                if (indexEquals != -1)
                    return data.Substring(indexEquals + 1);
                else
                    return null;
            }
            else
        ...
        private void SendToBrowser(Byte[] bSendData, ref Socket mySocket)
        {
            ...
            {
                System.Console.WriteLine("ERROR SendToBrowser:" + ex.StackTrace);
        ...
        private void SeparateOPERLOGData(string datastr, string SN)
        {
            try
            {
                ...
                foreach (string i in strlist)
                {
                    ...
                    else if (tmpstr.Split(' ')[0] == "USER")//处理用户信息  USERINFO
                    {
                        SaveUserinfo(tmpstr);
                    }
        ...
        private void SeparateBioData(string datastr, string SN)
        {
            try
            {
                ...
                foreach (string i in strlist)
                {
                    ...
                    Console.WriteLine("Biodata::biotype: " + bioType);
                    switch (bioType)
                    {
        ...
        {
            string machineSN = sBuffer.Substring(sBuffer.IndexOfEx("SN=") + 3);
            Console.WriteLine("Biodata::Machine: " + machineSN);
            string SN = machineSN.Split('&')[0];
            Console.WriteLine("Biodata::SN: " + SN);

            string machinestamp = sBuffer.Substring(sBuffer.IndexOfEx("Stamp=") + 6);
            Console.WriteLine("Biodata::MachineStamp: " + machinestamp);
            string Stamp = "";
            this.GetTimeNumber(machinestamp, ref Stamp); // Get TimeStamp 
        ...
        private void AttLogProcess(string attstr, string machineSN)
        {
            try
            {
                System.Console.WriteLine("En el proceso de ATTLOG");
        ...
        private CheckinoutModel CreateChecklog(AttLogModel attlog)
        {
            System.Console.WriteLine("Guardando checkinout .....");
            CheckinoutModel checkinoutModel = new CheckinoutModel();
            checkinoutModel.userid = Int32.Parse(attlog.PIN);
            checkinoutModel.checktime = attlog.AttTime;
            checkinoutModel.checktype = attlog.Status;
            checkinoutModel.verifycode = Int32.Parse(attlog.Verify);
            checkinoutModel.SN = attlog.DeviceID;
            checkinoutModel.sensorid = "0";
            checkinoutModel.WorkCode = attlog.WorkCode;
            checkinoutModel.Reserved = attlog.Reserved1;
            checkinoutModel.company_id = 0;
            CheckinoutBll.Add(checkinoutModel);
        ...
        {
            System.Console.WriteLine("En el proceso de SaveAttLog .....");
            AttLogModel attlogModel = null;
            if (OnNewAttLog != null)
            {
                attlogModel = CreateAttlog(attlog, machineSN);
                OnNewAttLog(attlogModel);
            }
            System.Console.WriteLine("SaveAttLog: " + attlogModel);
            if (attlogModel != null)
            {
                CreateChecklog(attlogModel);
            }
        }
        #endregion
        ...
        private void Base64StringToImage(string txtFileName)
        {
            ...
            catch (Exception ex)
            {
                ...
                Log.WriteLogs("Error de conversión de Base64: " + ex.Message);
            }
        }
